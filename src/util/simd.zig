...
/// `a <op> b` for a scalar or vector: two's-complement wrapping for integers
/// (the SQL integer arithmetic semantics, DESIGN.md §3.4), plain IEEE for floats.
inline fn applyOp(comptime op: BinOp, a: anytype, b: @TypeOf(a)) @TypeOf(a) {
    const Elem = switch (@typeInfo(@TypeOf(a))) {
        .vector => |v| v.child,
        else => @TypeOf(a),
    };
    if (@typeInfo(Elem) == .int) return switch (op) {
        .add => a +% b,
        .sub => a -% b,
        .mul => a *% b,
    };
...
/// Integer ops wrap; float ops are plain IEEE. `dst` must already be sized to
/// `a.len`.
pub fn binInto(comptime T: type, comptime op: BinOp, a: []const T, b: []const T, dst: []T) void {
    ...
    if (N > 1) {
        while (i + N <= dst.len) : (i += N) {
            ...
            dst[i..][0..N].* = applyOp(op, va, vb);
        }
    }
    while (i < dst.len) : (i += 1) dst[i] = applyOp(op, a[i], b[i]);
}
...
pub fn scalarOp(
    ...
    if (N > 1) {
        ...
        while (i + N <= dst.len) : (i += N) {
            ...
            dst[i..][0..N].* = if (col_left) applyOp(op, w, sv) else applyOp(op, sv, w);
        }
    }
    while (i < dst.len) : (i += 1) {
        const w: Tout = src[i];
        dst[i] = if (col_left) applyOp(op, w, scalar) else applyOp(op, scalar, w);
    }
}
...
test "simd: binInto and scalarOp wrap an overflowing lane anywhere in the slice" {
    const len = 37;
    inline for (.{ i32, i64 }) |T| {
        const cases = .{
            .{ .op = BinOp.add, .big = std.math.maxInt(T), .want = std.math.minInt(T) + 1 },
            .{ .op = BinOp.sub, .big = std.math.minInt(T), .want = std.math.maxInt(T) - 1 },
            .{ .op = BinOp.mul, .big = std.math.maxInt(T), .want = -2 },
        };
        inline for (cases) |c| {
            for ([_]usize{ 0, 5, len - 1 }) |at| {
                ...
                a[at] = c.big;
                binInto(T, c.op, &a, &b, &dst);
                try std.testing.expectEqual(@as(T, c.want), dst[at]);
                @memset(&dst, 0);
                scalarOp(T, T, c.op, true, &a, 2, &dst);
                try std.testing.expectEqual(@as(T, c.want), dst[at]);
            }
        }
    }
