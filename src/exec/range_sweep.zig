...
fn cmpCells(left: ColumnView, lrow: u32, right: ColumnView, rrow: u32) std.math.Order {
    switch (left.data) {
        ...
        .float => return types.floatOrder(left.data.float[lrow], right.data.float[rrow]),
        .double => return types.floatOrder(left.data.double[lrow], right.data.double[rrow]),
...
fn cmpInColumn(col: ColumnView, a: u32, b: u32) std.math.Order {
    return switch (col.data) {
        ...
        .float => types.floatOrder(col.data.float[a], col.data.float[b]),
        .double => types.floatOrder(col.data.double[a], col.data.double[b]),
