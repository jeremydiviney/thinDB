...
/// One per Table. Thread-safety: caller serializes (e.g., the Table mutex).
pub const WalWriter = struct {
    allocator: Allocator,
    io: Io,
    dir: Io.Dir,
    file: Io.File,
    /// True if our previous calls have fsynced — used purely for diagnostics.
    last_fsync_ok: bool = true,
    ...
    pub fn create(
        ...
        return .{ .allocator = allocator, .io = io, .dir = dir, .file = file };
    }
    ...
    /// as an insert record, append + fsync.
    pub fn appendInsert(self: *WalWriter, mt: *const Memtable, from: usize, to: usize) !void {
        if (from == to) return;
        ...
        for (mt.columns, mt.schema.columns) |col, schema_col| {
            try encodeColumnRange(self.allocator, &payload, col, schema_col, from, to);
        }

        try self.writeRecord(.insert, payload.items);
    }

    /// Encode a delete predicate as a record. Idempotent on replay.
    pub fn appendDelete(self: *WalWriter, pred: anytype) !void {
        var payload: std.ArrayList(u8) = .empty;
        defer payload.deinit(self.allocator);
        ...
        try encodeValue(self.allocator, &payload, pred.val);

        try self.writeRecord(.delete, payload.items);
    }

    pub fn appendFlushMarker(self: *WalWriter, max_segment_id: u64) !void {
        var payload: [8]u8 = undefined;
        format.writeU64(&payload, max_segment_id);
        try self.writeRecord(.flush_marker, &payload);
    }

    /// Truncate the WAL back to a fresh file (just the header). Called after
    /// a flush has been durably committed to the manifest.
    pub fn truncate(self: *WalWriter, schema_fingerprint: u64) !void {
        // Close, recreate (truncates), reopen.
        self.file.close(self.io);
    ...
    /// Write a record (header + payload + checksum) and fsync.
    fn writeRecord(self: *WalWriter, t: RecordType, payload: []const u8) !void {
        // Build the framed bytes in one buffer so we can compute the checksum
        // over the same bytes that go to disk.
...
pub fn replay(
    ...
    while (cursor < bytes.len) {
        ...
        switch (rec.type) {
            .insert => try applyInsertRecord(allocator, rec.payload, mt),
            .delete => try applyDeleteRecord(allocator, rec.payload, mt),
            .flush_marker => {},
        }
        did_replay = true;
...
fn applyInsertRecord(allocator: Allocator, payload: []const u8, mt: *Memtable) !void {
...
fn applyDeleteRecord(allocator: Allocator, payload: []const u8, mt: *Memtable) !void {
    _ = allocator;
    if (payload.len < 4) return Error.WalCorrupt;
    var cursor: usize = 0;
...
fn encodeValue(allocator: Allocator, out: *std.ArrayList(u8), v: Value) !void {
    var b: [16]u8 = undefined;
    switch (v) {
        .int => |x| {
