...
pub fn main() !void {
    ...
    if (getenv("THINDB_AB_UPSTREAM") != null) {
        ...
        const up_sql = try std.fmt.allocPrint(allocator, "SELECT COUNT(*) AS n FROM ({s}) _up", .{sqls.b[std.mem.indexOf(u8, sqls.b, "((").? + 2 .. std.mem.lastIndexOf(u8, sqls.b, ")) PARTITION BY").?]});
